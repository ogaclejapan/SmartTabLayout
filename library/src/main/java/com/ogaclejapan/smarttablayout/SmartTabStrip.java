...
class SmartTabStrip extends LinearLayout {
    ...
    // new variable for gradient indicator
    ...
    // new function to overiding gradient
    void getGradientColor(boolean isUsingGradient, int colorStart, int colorEnd) {
        usingGradientColor = isUsingGradient;
        colorGradientStart = colorStart;
        colorGradientEnd = colorEnd;
    }
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
        ...
        // creating new paint for gradient
        Paint paint = new Paint();
        paint.setStyle(Paint.Style.FILL);
        paint.setShader(new LinearGradient((float) left, (float) height, (float) right, (float) height, colorGradientStart, colorGradientEnd, Shader.TileMode.CLAMP));

        if (indicatorCornerRadius > 0f) {
            // if using gradient color, use new paint
            if (usingGradientColor) {
                canvas.drawRoundRect(
                        indicatorRectF, indicatorCornerRadius,
                        indicatorCornerRadius, paint);
            } else {
                canvas.drawRoundRect(
                        indicatorRectF, indicatorCornerRadius,
